...
/* ── KPI ROW ── */
.kpi-row{{display:grid;grid-template-columns:repeat(8,1fr);gap:10px;margin-bottom:28px}}
.kpi-card{{
  background:var(--surface);border:1px solid var(--border);
...
.kpi-tag{{font-family:var(--mono);font-size:9px;letter-spacing:1.5px;color:var(--muted);text-transform:uppercase;margin-bottom:6px}}
.kpi-val{{font-family:var(--mono);font-size:18px;font-weight:500;color:var(--ink);line-height:1;margin-bottom:3px}}
...
<div class="topbar">
  ...
  <div class="topbar-badges">
    <span class="tb-badge">历史消耗 {hist_date_range[0] or '2026/5/1'} – {hist_date_range[1] or '至今'}</span>
